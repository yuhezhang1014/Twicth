...
public class AppConfig {
    ...
    // filterChain 是一个用于配置 Spring Security 的 SecurityFilterChain
//    @Bean
//    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
//        http
//                .csrf().disable() // 禁用 CSRF（跨站请求伪造）保护，一般在非浏览器客户端的应用中禁用
...
//                .and()
//                .formLogin() // 配置表单登录
//                .successHandler((req, res, auth) -> res.setStatus(HttpStatus.NO_CONTENT.value())) // 登录成功时返回 HTTP 204 No Content
//                .failureHandler(new SimpleUrlAuthenticationFailureHandler()) // 登录失败时使用默认的失败处理器
//                .and()
//                .logout() // 配置登出
//                .logoutSuccessHandler(new HttpStatusReturningLogoutSuccessHandler(HttpStatus.NO_CONTENT)); // 登出成功时返回 HTTP 204 No Content
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .formLogin(form -> form // formLogin实际上实现的就是Session-based验证ID。下面都是在定义不同情况下的行为，防止它自己重定向，打开其他页面
                        .successHandler((req, res, auth) -> res.setStatus(HttpStatus.NO_CONTENT.value())) // 如果成功
                        .failureHandler(new SimpleUrlAuthenticationFailureHandler()) // 如果失败
                )
                .logout(logout -> logout
                        .logoutSuccessHandler(new HttpStatusReturningLogoutSuccessHandler(HttpStatus.NO_CONTENT)) // 如果成功
                );
        return http.build();
    }
